...
def self_paginator(request, List, Limit):
	"""分页模块,用法:
		1.view中引入：
		ex:from website.common.CommonPaginator import self_paginator

		2.SelfPaginator需要传入三个参数
			(1).request:获取请求数据
			(2).List:为需要分页的数据（一般为*.objects.all()取出来数据）
			(3).Limit:为每页显示的条数
		ex:lst = self_paginator(request,mList, 5)
...
if __name__ == '__main__':
	...
	lst = self_paginator(request, rList, 20)
